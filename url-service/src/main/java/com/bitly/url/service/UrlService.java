...
public class UrlService {
  ...
  public UrlResponse createShortUrl(CreateUrlRequest request, Long userId) {
    ...
    public String getLongUrl(String shortCode) {
      ...
      String longCode = urlRepository.findByShortCode(shortCode).map(this::toLongUrlResponse).orElseThrow(() -> new RuntimeException("Short code not found. " + shortCode));

      redisTemplate.opsForValue().set(REDIS_PREFIX + shortCode, longCode, Duration.ofHours(24));
      publishClickEvent(shortCode);
      return longCode;
    }

    private void publishClickEvent(String shortCode) {
      ClickEvent event =  ClickEvent.builder().shortCode(shortCode).timestamp(java.time.LocalDateTime.now()).build();
      kafkaTemplate.send(KAFKA_TOPIC, event);
      log.info("Published click event for shortCode: {}", shortCode);
    }
    ...
    private String toLongUrlResponse(Url url) {
